...
org_location_4 = [[29220, 49680], [59996, 37196], [61779, 36414], [17423, 39241], [50038, 29307]]
...
str_wrt = "PSNR, SSIM, VIF, PSNR_blended, SSIM_blended, VIF_blended, PSNR_stitch, SSIM_stitch, VIF_stitch\n"
fp = open(os.path.join(img_dir_out, "eval_result.csv"), 'w')
for idx, wsi_uuid in enumerate(wsi_uuid_list):
    print("processing case: %s" % wsi_uuid)
    case_dir = os.path.join(img_dir, wsi_uuid)
    for org_location in org_location_list[idx]:
        ...
        s = "%f,%f,%f,%f,%f,%f,%f,%f,%f\n" %(PSNR, SSIM, VIF, PSNR_b, SSIM_b, VIF_b, PSNR_s, SSIM_s, VIF_s)
        str_wrt += s
